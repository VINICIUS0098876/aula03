...
function identificar(){
    ...
    if(!(numero > 3)){
        ...
        resultado.textContent = 'Valor maior que 3'
    }
}
...
function calcular() {
    ...
    if (result2 >= 7) {
      ...
      if (isNaN(media5)) {
        resultado3.textContent = 'INSIRA A 5ª NOTA';
      } else if (result3 >= 5) {
...
function verifica(){
    const numero = Number(document.getElementById('numero1-item6').value)
    const resultado = document.getElementById('resultado-item6')

    if(numero % 2 === 0){
        resultado.textContent = 'PAR!'
    }else{
        resultado.textContent = 'INPAR!'
    }
}

botaoVerificar.addEventListener('click', verifica)
